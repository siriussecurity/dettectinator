...
class TestCli(unittest.TestCase):
    ...
    def test_csv(self):
        ...
        result = subprocess.run(['python', self.run_path, '-p', 'ImportCsv', '-a', 'all', '--file', import_filename], universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=self.output_dir)
    ...
    def test_csv_custom_output_filename(self):
        import_filename = os.path.join(self.output_dir, 'import.csv')
        shutil.copyfile(os.path.join(os.path.dirname(os.path.abspath(__file__).replace('tests', '')), 'examples/import.csv'), import_filename)
        result = subprocess.run(['python', self.run_path, '-p', 'ImportCsv', '-a', 'all', '--file', import_filename, '-o', 'test_csv.yaml'], universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=self.output_dir)
