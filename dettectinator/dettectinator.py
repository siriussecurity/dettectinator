...
def get_raw_commandline(argument_names: list) -> str:
    """
    We need to get the name from the plugin before Argument parser can do its job.
    This is because the plugin needs to be able to add its arguments to the command line
    before argument parser does its job. It's ugly, but it works :)
...
def print_plugins(import_plugins: dict) -> None:
    """
...
def get_plugins() -> dict:
    ...
    for module in [x for x in os.listdir(path) if x[-3:] == '.py']:
        ...
        for name, cls in inspect.getmembers(plugin_mod, inspect.isclass):
            if 'Import' in name and 'Base' not in name:
...
if __name__ == '__main__':
    ...
    config_file_name = get_raw_commandline(['-c', '--config'])
    ...
    plugin_name = config_file_arguments.get("plugin", get_raw_commandline(['-p', '--plugin']))
    ...
    if plugin_name:
        ...
        dettect = DettectTechniquesAdministration(arguments.input_file, domain=arguments.domain, local_stix_path=arguments.stix_location)
